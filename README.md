...
* Для построения хороших планов запросов, статистика должна быть актуальна
* Для анализа проблемных мест (не используется индекс, много seq scan по таблице)
* Для нахождения медленных запросов

## Немного про процесс сбора статистики
*Сборщик статистики немного увеличивает нагрузку на сервер*
...
* track_io_timing - включает мониторинг времени чтения и записи блоков. По умолчанию off так как для этого требуется
постоянно запрашивать текущее время у операционной системы, что может значительно замедлить работу на некоторых
платформах
* track_wal_io_timing - включает мониторинг времени записи WAL (может значительно замедлять работу на ASTRA Linux и других OS)

*autovacuum при очистке вызывает сбор статистики*

## Задать значение срабатывания сбора статистики
Default_statistics_target = N (default value 100)
